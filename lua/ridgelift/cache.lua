...
local M = {}
local theme_cache_file = vim.fn.stdpath("cache") .. "/ridgelift_cache.json"
...
--- build a cache file for a configured ridgelift theme
--- @param theme table
...
--- load a cache file for a configured ridgelift theme
M.load = function()
    local cache = io.open(theme_cache_file, "r")
    if not cache then
        M.build(require("ridgelift.theme").setup())
